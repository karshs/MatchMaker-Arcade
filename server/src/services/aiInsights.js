...
async function generateInsights(customerA, customerB) {
  ...
  if (!genai || config.aiMode !== 'gemini') {
    throw new Error('AI Insights are not configured properly. Please check your API keys.');
  }
  ...
  try {
    ...
    throw new Error('Failed to generate AI insights from Google Gemini: ' + err.message);
  }
}
